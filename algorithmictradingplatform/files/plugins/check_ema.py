...
cmd_arg_help = "This plugin checks the exponential moving average (EMA) for a stock. Based on the output, you can decide to set certain warning and critical threshold levels."
...
if __name__ == "__main__":
    ...
    ema = np.round(dataFrame.ewm(span=emaPeriod,adjust=False).mean().iloc[-1], 2)
    ...
    if args.raw:
        ...
        if args.max and int(args.max) <  ema:
            print("WARNING - EMA($" + str(ema) + ") is above threshold " + str(args.max))
            returnCode = WARNING
        elif args.min and int(args.min) >  ema:
            print("WARNING - EMA($" + str(ema) + ") is below threshold " + str(args.min))
            returnCode = WARNING
        else:
            print("OK - $" + str(ema))
